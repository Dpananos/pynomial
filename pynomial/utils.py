...
def _check_args(x, n, conf):
    ...
    if (isinstance(x, np.ndarray) and not issubclass(x.dtype.type, np.integer)) or (
        isinstance(x, float) and not x.is_integer()
    ):
        raise ValueError("x must be an integer or an array of integers")

    elif (isinstance(n, np.ndarray) and not issubclass(n.dtype.type, np.integer)) or (
        isinstance(n, float) and not n.is_integer()
    ):
